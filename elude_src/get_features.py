...
# A = pickle.load(open('ade20k_imagelabels.pth', 'rb'))
A = torch.load('data/ade20k/ade20k_imagelabels.pth')
for name, m in [
    ...
    with torch.no_grad():
        ...
        for split in ['train', 'val', 'test']:
            ...
            save_dir = 'ADE20k_elude'
            ensure_dir(save_dir)
            pickle.dump(img_to_feature, open('{}/{}_{}.pkl'.format(save_dir, split, name), 'wb+'))
            if name=='logits':
                pickle.dump(img_to_scene, open('{}/{}_scene.pkl'.format(save_dir, split), 'wb+'))
